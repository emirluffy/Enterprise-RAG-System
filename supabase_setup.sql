...
-- Create Learned Knowledge table
CREATE TABLE IF NOT EXISTS learnedknowledge (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    content TEXT NOT NULL,
    ...
    learned_from_query UUID REFERENCES querylog(id),
    embedding JSONB DEFAULT '{}',
    ...
    status VARCHAR(20) DEFAULT 'active',
    superseded_by UUID REFERENCES learnedknowledge(id),
...
-- Create Query-Document Link table (many-to-many)
CREATE TABLE IF NOT EXISTS querydocumentlink (
...
CREATE INDEX IF NOT EXISTS idx_learnedknowledge_created_by ON learnedknowledge(created_by);
CREATE INDEX IF NOT EXISTS idx_learnedknowledge_department ON learnedknowledge(department);
...
CREATE TRIGGER update_learnedknowledge_updated_at BEFORE UPDATE ON learnedknowledge FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
